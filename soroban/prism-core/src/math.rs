...
pub fn q_to_u64(q: u128) -> Result<u64, PrismError> {
...
pub fn mul_div_q(a: u64, b_q: u128, denom_q: u128) -> Result<u128, PrismError> {
    if denom_q == 0 {
        return Err(PrismError::EmptyTrancheNav);
    }
...
pub fn deposit_shares(usdc_in: u64, nav_q: u128, total_supply: u64) -> Result<u64, PrismError> {
    if total_supply == 0 {
        return Ok(usdc_in);
    }
    if nav_q == 0 {
        return Err(PrismError::TrancheWipedNoDepositsAllowed);
    }
...
pub fn withdraw_payout(shares: u64, nav_q: u128) -> Result<u64, PrismError> {
...
mod tests {
    ...
    fn test_deposit_shares_against_blocked_wiped_tranche() {
        ...
        assert_eq!(err, PrismError::TrancheWipedNoDepositsAllowed);
    }

    #[test]
    fn test_round_trip_deposit_then_withdraw_at_nav_one() {
        // NAV = 1.0; depositing 1_000_000 → 1_000_000 shares; withdrawing them returns ~1_000_000.
