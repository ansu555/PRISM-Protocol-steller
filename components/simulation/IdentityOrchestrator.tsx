...
import { formatUsdc, shortKey, toBigInt } from '@/app/lib/format';
...
export function IdentityOrchestrator() {
  ...
  const balances = useQuery({
    queryKey: ['identity-balances', Object.values(identities).map((identity) => identity.address).join(':')],
    ...
    queryFn: async () => {
      ...
      await Promise.all(
        (Object.keys(identities) as Role[]).map(async (key) => {
          ...
          const usdc = await getUsdcClient()
            .read<bigint | number | string>('balance', [addr(identity.address)])
            .then(toBigInt)
            .catch(() => 0n);
          result[key] = { xlm: 0, usdc };
        }),
      );
